...
def optimize_flags_sequence(
    ...
    require_radare2: bool = True,
    cleanup_intermediates: bool = True,
    stream_radare2_logs: bool = True,
) -> Dict[str, object]:
    """Sequentially evaluate flags and return the optimization summary.

    The helper enforces radare2-based scoring (falling back to an explicit error
    if radare2 is unavailable), streams radare2 logs when requested, and prunes
    intermediate binaries so the caller receives only the baseline build plus the
    most obfuscated result."""
    ...
    try:
        summary = optimize_flags(
            ...
            stream_radare2_logs=stream_radare2_logs,
        )
...
def optimize(
    ...
    log_radare2: bool = typer.Option(True, "--log-radare2/--no-log-radare2", help="Stream radare2 command output during evaluation."),
) -> None:
    ...
    tool_args = {
        ...
        "require_radare2": True,
        "cleanup_intermediates": True,
        "stream_radare2_logs": log_radare2,
    }
